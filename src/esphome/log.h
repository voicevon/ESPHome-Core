#ifndef ESPHOME_LOG_H
#define ESPHOME_LOG_H
...
#else
  #define ESPHOME_LOG_COLOR_E
  #define ESPHOME_LOG_COLOR_W
  #define ESPHOME_LOG_COLOR_I
  #define ESPHOME_LOG_COLOR_C
  #define ESPHOME_LOG_COLOR_D
  #define ESPHOME_LOG_COLOR_V
  #define ESPHOME_LOG_COLOR_VV
...
int esp_idf_log_vprintf_(const char *format, va_list args);
...
#else
  #define ESPHOME_LOG_FORMAT(tag, letter, format)  ESPHOME_LOG_COLOR_ ## letter "[" #letter "][%s:%03u]: " format ESPHOME_LOG_RESET_COLOR, tag, __LINE__
#endif

#if ESPHOME_LOG_LEVEL >= ESPHOME_LOG_LEVEL_VERY_VERBOSE
  #define esph_log_vv(tag, format, ...) esp_log_printf_(ESPHOME_LOG_LEVEL_VERY_VERBOSE, tag, ESPHOME_LOG_FORMAT(tag, VV, format), ##__VA_ARGS__)
  ...
  #define esph_log_v(tag, format, ...) esp_log_printf_(ESPHOME_LOG_LEVEL_VERBOSE, tag, ESPHOME_LOG_FORMAT(tag, V, format), ##__VA_ARGS__)
  ...
  #define esph_log_d(tag, format, ...) esp_log_printf_(ESPHOME_LOG_LEVEL_DEBUG, tag, ESPHOME_LOG_FORMAT(tag, D, format), ##__VA_ARGS__)

  #define esph_log_config(tag, format, ...) esp_log_printf_(ESPHOME_LOG_LEVEL_DEBUG, tag, ESPHOME_LOG_FORMAT(tag, C, format), ##__VA_ARGS__)
  ...
  #define esph_log_i(tag, format, ...) esp_log_printf_(ESPHOME_LOG_LEVEL_INFO, tag, ESPHOME_LOG_FORMAT(tag, I, format), ##__VA_ARGS__)
  ...
  #define esph_log_w(tag, format, ...) esp_log_printf_(ESPHOME_LOG_LEVEL_WARN, tag, ESPHOME_LOG_FORMAT(tag, W, format), ##__VA_ARGS__)
  ...
  #define esph_log_e(tag, format, ...) esp_log_printf_(ESPHOME_LOG_LEVEL_ERROR, tag, ESPHOME_LOG_FORMAT(tag, E, format), ##__VA_ARGS__)
